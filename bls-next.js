function blsSetup (mod, getRandomValues, ethMode = true) {
  const BN254 = 0
  const BN381_1 = 1
  const BLS12_381 = 5
...
//////////////////////////////////
  ...
  */
  ...
  // change curveType
  const blsInit = (curveType = ethMode ? BLS12_381 : BN254) => {
    const r = mod._blsInit(curveType, MCLBN_COMPILED_TIME_VAR)
    if (r) throw ('blsInit err ' + r)
  }
  const getCurveOrder = _wrapGetStr(mod._blsGetCurveOrder)
  const getFieldOrder = _wrapGetStr(mod._blsGetFieldOrder)

  const blsIdSetDecStr = _wrapInput(mod._blsIdSetDecStr, 1)
  const blsIdSetHexStr = _wrapInput(mod._blsIdSetHexStr, 1)
  const blsIdGetDecStr = _wrapGetStr(mod._blsIdGetDecStr)
  const blsIdGetHexStr = _wrapGetStr(mod._blsIdGetHexStr)

  const blsIdSerialize = _wrapSerialize(mod._blsIdSerialize)
  const blsSecretKeySerialize = _wrapSerialize(mod._blsSecretKeySerialize)
  const blsPublicKeySerialize = _wrapSerialize(mod._blsPublicKeySerialize)
  const blsSignatureSerialize = _wrapSerialize(mod._blsSignatureSerialize)

  const blsIdDeserialize = _wrapDeserialize(mod._blsIdDeserialize)
  const blsSecretKeyDeserialize = _wrapDeserialize(mod._blsSecretKeyDeserialize)
  const blsPublicKeyDeserialize = _wrapDeserialize(mod._blsPublicKeyDeserialize)
  const blsSignatureDeserialize = _wrapDeserialize(mod._blsSignatureDeserialize)

  const blsPublicKeySerializeUncompressed = _wrapSerialize(mod._blsPublicKeySerializeUncompressed)
  const blsSignatureSerializeUncompressed = _wrapSerialize(mod._blsSignatureSerializeUncompressed)
  const blsPublicKeyDeserializeUncompressed = _wrapDeserialize(mod._blsPublicKeyDeserializeUncompressed)
  const blsSignatureDeserializeUncompressed = _wrapDeserialize(mod._blsSignatureDeserializeUncompressed)

  const blsSecretKeySetLittleEndian = _wrapInput(mod._blsSecretKeySetLittleEndian, 1)
  const blsSecretKeySetLittleEndianMod = _wrapInput(mod._blsSecretKeySetLittleEndianMod, 1)
  const blsHashToSecretKey = _wrapInput(mod._blsHashToSecretKey, 1)
  const blsSign = _wrapInput(mod._blsSign, 2)
  const blsVerify = _wrapInput(mod._blsVerify, 2, true)
  ...
  const deserializeHexStrToId = s => {
    const r = new Id()
    r.deserializeHexStr(s)
    return r
  }

  class SecretKey extends Common {
    constructor () {
  ...
  const deserializeHexStrToSecretKey = s => {
    const r = new SecretKey()
    r.deserializeHexStr(s)
    return r
  }

  class PublicKey extends Common {
    constructor () {
  ...
  const deserializeHexStrToPublicKey = s => {
  ...
  const deserializeHexStrToSignature = s => {
    const r = new Signature()
    r.deserializeHexStr(s)
    return r
  }
  // 1 (draft-05) 2 (draft-06) 3 (draft-07)
  const setETHmode = (mode) => {
  ...
  const verifySignatureOrder = (doVerify) => {
    mod._blsSignatureVerifyOrder(doVerify)
  }
  // make setter check the correctness of the order if doVerify
  const verifyPublicKeyOrder = (doVerify) => {
    mod._blsPublicKeyVerifyOrder(doVerify)
  }
  const areAllMsgDifferent = (msgs, msgSize) => {
  ...
  */
  const multiVerify = (pubs, sigs, msgs) => {
    const MSG_SIZE = 32
    ...
    getRandomValues(mod.HEAP8.subarray(randPos, randPos + RAND_SIZE * n))
    for (let i = 0; i < n; i++) {
...
if (typeof exports === 'object' && typeof module === 'object')
  module.exports = blsSetup;
else if (typeof define === 'function' && define['amd'])
  define([], function() { return blsSetup; });
else if (typeof exports === 'object')
  exports["blsSetup"] = blsSetup;
